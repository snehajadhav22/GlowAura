...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'py-2' : 'py-4'}`}>
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-pink-400 via-purple-400 to-pink-400 py-1.5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-2">
        <div className={`bg-[var(--glass-bg)] backdrop-blur-2xl rounded-[2rem] border border-[var(--glass-border)] shadow-xl transition-all duration-500 px-6 py-3 flex items-center justify-between gap-4`}>
          
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 group flex-shrink-0">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-pink-400 to-purple-50 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all">
              <Sparkles size={20} className="text-white" />
            </div>
            <span className="text-2xl font-black font-playfair bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent hidden sm:block">
              GlowAura
            </span>
          </Link>
          ...
          <div className="hidden lg:flex flex-1 max-w-md relative group">
            ...
            <div className="relative w-full flex items-center bg-gray-50/50 dark:bg-white/5 border border-pink-100 dark:border-white/10 rounded-2xl px-4 py-1.5 focus-within:border-pink-400 focus-within:bg-white dark:focus-within:bg-black/40 transition-all">
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            <button 
              ...
              className="w-10 h-10 rounded-2xl bg-pink-50 dark:bg-white/5 flex items-center justify-center text-pink-500 dark:text-pink-400 hover:scale-110 active:scale-95 transition-all"
            >
              {isDark ? <Sun size={20} /> : <Moon size={20} />}
            </button>

            <Link href="/wishlist" className="hidden sm:flex w-10 h-10 rounded-2xl bg-pink-50 dark:bg-white/5 items-center justify-center text-pink-500 dark:text-pink-400 hover:scale-110 active:scale-95 transition-all">
              <Heart size={20} />
            </Link>

            <Link href="/cart" className="w-10 h-10 rounded-2xl bg-pink-50 dark:bg-white/5 flex items-center justify-center text-pink-500 dark:text-pink-400 hover:scale-110 active:scale-95 transition-all relative">
              <ShoppingBag size={20} />
              {items.length > 0 && (
            ...
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-2 p-1.5 pl-3 bg-pink-50 dark:bg-white/5 rounded-2xl hover:bg-pink-100 dark:hover:bg-white/10 transition-all border border-pink-100 dark:border-white/10"
              >
                <div className="hidden sm:block text-right">
                  <p className="text-[10px] font-black uppercase text-pink-500 tracking-tight leading-none mb-1">Welcome!</p>
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-200 leading-none truncate max-w-[80px]">{user?.name ? user.name.split(' ')[0] : 'Bestie'}</p>
                </div>
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-pink-400 to-purple-500 flex items-center justify-center text-white shadow-md">
                  <User size={18} />
                </div>
              </button>

              {isUserMenuOpen && (
                <div className="absolute top-14 right-0 w-64 bg-white/95 dark:bg-black/80 backdrop-blur-2xl rounded-3xl border border-pink-50 dark:border-white/10 shadow-2xl p-3 animate-in fade-in slide-in-from-top-4 duration-300">
                  {user?.email ? (
                    <>
                    ...
                    <div className="space-y-3 p-2">
                      <div className="text-center pb-2">
                        ...
                        <p className="text-[10px] text-gray-500 mt-1">Unlock exclusive deals & virtual try-on saves!</p>
                      </div>
                      <Link 
                        href="/login" 
                        onClick={() => setIsUserMenuOpen(false)}
            ...
            <button 
              ...
              className="lg:hidden w-10 h-10 rounded-2xl flex items-center justify-center text-pink-500 transition-all active:scale-90"
            >
        ...
        <div className="hidden lg:flex items-center gap-4 mt-4 px-2 overflow-x-auto scrollbar-hide">
          <Link href="/ar-studio" className="bg-gradient-to-r from-pink-500 to-purple-600 text-white px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-pink-200">
            <Sparkles size={14} /> AR Studio Live
          </Link>
          <div className="w-px h-6 bg-pink-100 mx-2" />
          <Link href="/shop" className="bg-pink-100/50 text-pink-600 px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-pink-100 transition-all">
            All ✨
          </Link>
          {BRANDS.map(brand => (
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white dark:bg-black/95 backdrop-blur-3xl border-t border-pink-50 dark:border-white/10 p-6 animate-in slide-in-from-top duration-500">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <Link href="/shop" className="flex items-center gap-3 p-4 bg-pink-50 dark:bg-white/5 rounded-2xl">
                 <ShoppingBag className="text-pink-500" />
                 <span className="font-black uppercase tracking-tight text-xs">Shop</span>
              </Link>
              <Link href="/ar-studio" className="flex items-center gap-3 p-4 bg-purple-50 dark:bg-white/5 rounded-2xl">
            ...
            <div className="space-y-4">
              ...
              {['New In', 'Trending', 'Accessories', 'About'].map(link => (
                <Link key={link} href="#" className="block text-xl font-black text-gray-800 dark:text-gray-200 hover:text-pink-500 transition-colors">
                  {link}
                </Link>
              ))}
            </div>
